...
- **Storage**: S3 data bucket `tfg-data-<account>-<region>` (public access fully blocked, 7-day expiry on `raw/`).
- **Secrets**: random 24-character Postgres password (`pulumi-random`) stored in SSM as `/tfg/postgres_password`.
- **IAM**:
  - *Instance role*: read/write on the data bucket, read the SSM parameters under `/tfg/` (database password, AEMET key), SSM agent permissions.
  - *`tfg-gha-deploy`* (GitHub Actions): upload the deployment bundle to `bundle/*` in S3, write **only** the `/tfg/aemet_api_key` parameter, `ssm:SendCommand` only to instances tagged `Project=tfg`, read command results.
...
- Postgres runs in a container on the VM's disk (no managed backups). RDS would add durability at extra cost.
