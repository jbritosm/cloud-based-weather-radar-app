...
- **Satellite layers (EUMETSAT):** infrared, blended precipitation and cloud-top height are drawn straight from EUMETSAT's public **WMS** as raster tiles: no API key, CORS enabled, no backend involved. The user toggles layers and sets their opacity in the sidebar.
