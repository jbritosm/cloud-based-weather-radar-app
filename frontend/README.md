...
- **Security (Content Security Policy):** the production page tells the browser which scripts, images and connections it may use, and the browser refuses anything else, which limits the damage of an injected script or a compromised third party. The policy lives in one file, `csp.ts`, and `vite.config.ts` injects it into `index.html` **in the production build only** (Vite's dev server needs inline scripts for hot reload), so the browser tests and production run under exactly the same policy. Scripts: our own origin only, no inline scripts and no `eval` (which is why the theme is set by the external file `public/theme-init.js`, not an inline script). Connections and images: only the four hosts the app really uses (EUMETSAT, RainViewer's API and tile cache, OpenStreetMap). The one deliberate compromise is `style-src 'unsafe-inline'`, because React and MapLibre set style attributes; it affects styles, never scripts. The remaining security headers (HSTS, `X-Frame-Options`, `Permissions-Policy`...) are sent by Caddy (see `deploy/README.md`).
...
- **Unit (33 tests):** the time logic (parsing WMS extents, snapping to each layer's own cadence, never asking for the future), the radar frame selection, the shareable-link parser (including hostile input), that both languages have exactly the same texts and cover every layer, and the content security policy (no inline scripts, no eval, only the four external hosts the app uses). Writing them found a real bug: `?lat=&lng=&z=` (empty values) was accepted as a view at 0°, 0°, zoom 0.
- **Browser (36 tests, Chromium):** real user flows against the production build, on desktop and on a phone-sized screen: animation play/pause, "Now", layers and quick views, legends, dragging the map, shareable links, language, dark mode (system preference, switch, persistence, legend readability), a failing layer showing a notice, the mobile drawer and that the controls never cover the map attribution, and the PWA (manifest and icons valid, service worker installs, the shell opens **offline**, `/api` and other origins are never cached). Plus two groups described next: security and accessibility.
- **Security tests (4):** the production page carries the policy; it has no inline script; **a whole session** (every layer switched on, animation, dark mode, language switch, dragging, system-status panel) causes **zero policy violations** (measured with the browser's `securitypolicyviolation` event and console); and the app contacts **exactly** the allowed hosts and nothing else.
- **Accessibility tests (6, axe-core, WCAG 2.0/2.1 A and AA):** the main screen in Spanish, in dark mode, in English, and with every layer, legend and the status panel open must have no violations; there is one `main` landmark, one `h1` and a valid `lang`; and everything can be reached with the Tab key (the play button and language switch among the stops) and used with the keyboard. **They found two real defects, now fixed:** the blue text on the pale-blue "latest" badge and selected chip had a contrast of 4.49:1 (the minimum is 4.5:1), and the map attribution link was distinguished only by colour. Automated checks catch the mechanical problems; they do not replace trying the site with a screen reader.
